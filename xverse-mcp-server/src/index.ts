...
server.tool(
  "get_btc_balance",
  "Get confirmed and unconfirmed BTC balance for a Bitcoin address",
  {
...
server.tool(
  "get_btc_utxos",
  "Get confirmed UTXOs for a Bitcoin address",
  {
...
server.tool(
  "get_ordinals_inscriptions",
  "List all confirmed Ordinals inscriptions owned by a Bitcoin address",
  {
    address: z.string().min(10).describe("Bitcoin address"),
    offset: z.number().optional().default(0).describe("Pagination offset"),
  ...
  async ({ address, offset, limit }) => {
    const data = await xverseGet(
      `/v1/ordinals/address/${address}/inscriptions?offset=${offset}&limit=${limit}`
    );
    return {
      content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }],
    };
...
server.tool(
  "get_runes_balances",
  "Get Runes token balances for a Bitcoin address",
  {
    address: z.string().min(10).describe("Bitcoin address"),
    offset: z.number().optional().default(0).describe("Pagination offset"),
    limit: z.number().optional().default(60).describe("Results limit"),
  ...
  async ({ address, offset, limit }) => {
    const data = await xverseGet(
      `/v1/ordinals/address/${address}/runes?offset=${offset}&limit=${limit}`
    );
    return {
      content: [{ type: "text" as const, text: JSON.stringify(data, null, 2) }],
    };
...
server.tool(
  ...
  "Register a Bitcoin address for portfolio tracking. Must be called before fetching portfolio data.",
  {
  ...
  async ({ address, assetType }) => {
    ...
    return {
      content: [{ type: "text" as const, text: JSON.stringify(data ?? { status: "registered" }, null, 2) }],
    };
  }
);
...
server.tool(
  "get_btc_address_summary",
  "Get a comprehensive summary of a Bitcoin address including balance, transaction count, and funded/spent totals",
  {
    address: z.string().min(10).describe("Bitcoin address"),
  },
  async ({ address }) => {
