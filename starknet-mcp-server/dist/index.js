...
function getContract(account) {
    if (!VAULT_CONTRACT_ADDRESS)
        throw new Error("VAULT_CONTRACT_ADDRESS not set");
    const contract = new Contract(VAULT_ABI, VAULT_CONTRACT_ADDRESS, account || provider);
    return contract;
}
...
// 1. propose_action
server.tool("propose_action", "Propose an action to the AgentVault contract on Starknet. Returns the action_id.", {
...
// 2. approve_action
server.tool("approve_action", "Approve (validate constraints and execute) a proposed action. Returns whether approval succeeded.", {
...
// 3. get_agent_state
server.tool("get_agent_state", "Read the current AgentVault state: agent address, daily spent, constraints, total actions.", {}, async () => {
...
// 4. get_action
server.tool("get_action", "Read a specific action record by ID from the AgentVault.", {
...
server.tool("update_constraints", "Update vault constraints (owner only). Controls spending limits, allowed actions, risk threshold.", {
...
// 6. get_vault_summary
server.tool("get_vault_summary", "Get a comprehensive summary of the vault state including recent actions.", {
    recent_count: z.number().int().min(0).max(50).optional().default(5).describe("Number of recent actions to include"),
}, async ({ recent_count }) => {
    const contract = getContract();
    // Get state
    const stateResult = await contract.call("get_agent_state", []);
    ...
    for (let i = startIdx; i < totalActions; i++) {
        try {
            const action = await contract.call("get_action", [i]);
            actions.push({
                id: i,
